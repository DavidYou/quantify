...
def getData2(t):
    ...
    annual_volatility = volatility * np.sqrt(252 * 4 * 4 * 3)
...
def getData(t):
    ...
    annual_volatility = volatility * np.sqrt(252 * 4 * 4)
...
table = PrettyTable()
table.field_names = ["Code", "Name", "volatility"]

# 输出排序后的字典
for tt in sorted_dict:
    table.add_row([tt['ticker'], tt['name'], tt['volatility']])
